...
import { DataInputWidget } from './mycomps';


@Component({
    selector: 'wizard-step',
    template: `
    <div [hidden]="hidden" [ngClass]="'ui-Wizard ui-widget ui-helper-reset'" [ngStyle]="style" [class]="styleClass" role="tablist">
...
export class WizardStep implements AfterContentInit, OnInit {
    ...
    @ContentChildren("dw") alerts: QueryList<DataInputWidget>
  
    ngAfterContentInit() {
      console.log("called")
      this.alerts.forEach(ai =>{
        ai.stepComplete.subscribe(x => this.selectedChange.emit(x))
        this.hidden = true
      } );
...
@Component({
    selector: 'wizard',
    template: `
        <div [ngClass]="'ui-widget ui-helper-reset'" [ngStyle]="style" [class]="styleClass" role="tablist">
            <ng-content></ng-content>
        </div>
    `
})
export class Wizard implements  AfterContentInit, OnDestroy {
    ...
    ngAfterContentInit() {
        ...
        this.tabList.forEach(tab =>{
            tab.selectedChange.subscribe(x => {
              this._activeIndex++;  
